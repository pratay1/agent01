...
def run_training(args):
    ...
    console.print("[white on black]ChessAI Training Engine v3.0[/]")
    console.print("[dim]Press ESC to stop training, save model, and build new engine[/]")
    console.print()
    ...
    trainer = Trainer(network, buffer, learning_rate=0.002, weight_decay=1e-4)
    ...
    game_state = {
        ...
        "game_progress": 0.0,
        "games_in_epoch": 0,
        "total_in_epoch": args.games_per_epoch
    }
    ...
    def make_dashboard():
        game_bar = "=" * int(game_state['game_progress'] * 30) + "-" * (30 - int(game_state['game_progress'] * 30))
        ...
        t.add_row(
            f"[white]Epoch:[/] [cyan]{game_state['epoch']}[/]",
            f"[white]Mode:[/] [yellow]{game_state['mode']}[/]",
            f"[white]Elo:[/] [green]{game_state['elo']}[/]"
        )
        t.add_row(
            f"[white]Total Games:[/] {game_state['games']}",
            f"[white]Buffer:[/] [cyan]{game_state['buffer_pct']:.1%}[/]",
            f"[white]MCTS Sims:[/] [magenta]{game_state['sims']}[/]"
        )
        t.add_row(
            f"[white]Policy:[/] [green]{game_state['policy_loss']:.4f}[/]",
            f"[white]Value:[/] [yellow]{game_state['value_loss']:.4f}[/]",
            f"[white]Combined:[/] [cyan]{game_state['combined_loss']:.4f}[/]"
        )
        t.add_row(
            f"[white]Epoch Time:[/] [green]{game_state['epoch_time']:.1f}s[/]",
            f"[white]Best Elo:[/] [yellow]{best_elo}[/]",
            f"[white]Games/Epoch:[/] {args.games_per_epoch}"
        )

        progress_bar = f"[cyan]{game_bar}[/] {game_state['games_in_epoch']}/{game_state['total_in_epoch']}"

        game_panel = Panel(
            f"[white]Current Move:[/] [cyan]{game_state['current_move']}[/]\n"
            f"[white]Game Progress:[/] {progress_bar}",
            title="[white]Current Game[/]",
            border_style="white",
            padding=(1, 2)
        )
        ...
        main_panel = Panel(
            t,
            title="[white]Training Dashboard[/]",
            border_style="white",
            padding=(1, 2)
        )

        return Group(main_panel, game_panel)

    def on_move(info):
        game_state['current_move'] = info['move_num']
        game_state['sims'] = info['simulations']

    console.print(f"[dim]Config: {args.simulations} sims, {args.games_per_epoch} games/epoch, 500ms max move[/]")
    
    try:
        with Live(make_dashboard(), console=console, refresh_per_second=10, transient=False) as live:
            while True:
                if kb_hit():
                    key = get_key()
                    if key and key == b'\x1b':
                        console.print("\n[white]>>> ESC pressed! Stopping training...[/]")
                        break
                ...
                for g in range(args.games_per_epoch):
                    ...
                    game_state['game_progress'] = g / args.games_per_epoch
                    ...
                    game_state['games_in_epoch'] = g + 1
                    game_state['game_progress'] = (g + 1) / args.games_per_epoch
    ...
    console.print("\n[green]Done! New engine is ready at build/ChessEngine.exe[/]")
